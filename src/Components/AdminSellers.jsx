...
const ProductDetails = ({ Pid, Email, Mob, fName, lName, BName, userType, City, Country, Allow, PIN, userName, Address, Data, joining }) => {
    const [modalIsOpen, setModalIsOpen] = useState(false);

    const joining_date = joining.substring(0, joining.indexOf("T"));

    const decline = () => {
        axios.delete(`https://trisquare.asia/api/user_data/delete/${Pid}`).then((res) => alert("user removed."))
        ...
        <div className="admin__container">
            <div className="admin__products admin__userVerify">
                <div className="admin__productAbout" onClick={() => setModalIsOpen(true)} style={{ cursor: "pointer" }}>
                    <p>{`${fName} ${lName} registered as a ${userType} in your site on ${joining_date}.`}</p>
                </div>
                ...
                >
                    <div className="adminModal admin__userModalHeader">
                        <h1 className="orderModal__heading">New Seller Details</h1>
                        <button
                    ...
                    <div className="orderModal__userDescription">
                        ...
                        <div className="userDetails__rows"><h5 className="userDetalis__header">Joining :- </h5><span className="userDetails__span">{joining_date}</span></div>
                    </div>
                </Modal>
            </div>
        </div>)
        ...
        return (
            <div className="admin">
                ...
                <div className="admin__navbar extraNavbar">
                    ...
                    <Link to='/buyer/admin/sellers' className="admin__navbarOption active"><h4>All Sellers</h4></Link>
                ...
                <div className="admin__mainPage">
                    <h4 className="admin__heading">New Seller Details</h4>
                    {data && data.map((data) =>
                        data.approved == 1 ? data.user_type === "Seller" ?
                            (
                                <ProductDetails key={data.id} Pid={data.id} userName={data.username} Address={data.address} fName={data.first_name} lName={data.last_name} Mob={data.mobile} Country={data.country} PIN={data.pin} BName={data.business} Email={data.email} userType={data.user_type} City={data.city} Allow={data.allow} Data={data} joining={data.joiningDate} />) : "" : ""
                    )
                    }
                </div>
            </div>
